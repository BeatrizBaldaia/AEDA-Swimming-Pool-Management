...
MenuResult ViewAttendance::handle() {
	...
	MenuResult
	CustomerMakeCurrentBill::handle()
	{
		...
		bill
				...
				<< "                                   Bill of " << month << "/"
				<< year << endl << endl << endl;
		bill << "Lessons assisted: " << customerGivenLessons.size() << endl
				<< endl;
		...
		for (PoolUse * p : customerFreeSwimUses) {
			if (p->getDuration() < 100)
				bill << p->getDuration() << " minutes (" << p->getDate()
						<< ")..........................................€"
						<< fixed << setprecision(2) << p->getCost() << endl;
			else
				bill << p->getDuration() << " minutes (" << p->getDate()
						<< ").........................................€"
						<< fixed << setprecision(2) << p->getCost() << endl;
		...
		bill << "\n\n                            Total:   €" << fixed
				<< setprecision(2)
				<< c->getMonthCost(month, year)
						+ 3 * customerGivenLessons.size();
		return CONTINUE;
	}
	...
	RemoveLesson::handle()
	{
		...
		for (Lesson & i : pool.getSchedule()) {
			indice++;
			cout << indice << " - " << i.getModality() << endl
					<< i.getDayOfWeek() << " - " << i.getTime() << endl;
		}
		cout << "\n0 - Cancel\n";
	...
	MenuResult
	ViewTeacherGivenLessons::handle()
	{
		int ID;
		cout << "\nInsert teacher's ID: ";
		cin >> ID;
	...
	FreeSwimming::handle()
	{
		...
		pool.addFreeSwim(pool.getCustomer(ID), getCurrentDate(),
	...
	MenuResult
	ViewCustomersInformation::handle()
	{
		unsigned int ID;
		cout << "\nInsert customer's ID: ";
		cin >> ID;
